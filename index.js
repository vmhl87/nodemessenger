...
const port = (choosePort?parseInt(prompt('What port?')):process.env.PORT)
const qlog = (a) => {console.log(a);io.emit('block',-1,a)}
var a = 0
var sockets = []
var msgs = []
var users = []
var pwds = [{u:'',p:null},{u:'ADMIN',i:-1,p:'abcdabcd'}]
var blockedUsers = []
var pmblocked = []
...
io.on('connection', (socket) => {
  ...
  socket.on('chat message', (msg, user, pwd, room, id) => {
    ...
    if(msgsq==50){
       msgsq=0
      io.emit('back','This chat room was written by <a href="https://github.com/vmhl87">vmhl87</a>',room)
    }
